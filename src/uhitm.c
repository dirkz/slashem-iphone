...
#ifdef ENFORCER
/* as well as for the way of the Enforcer */
	if (Role_if(PM_ENFORCER) && mtmp->mpeaceful &&
	    u.ualign.record > -10) {
	    You("violate the way of the Enforcer!");
	    adjalign(-1);
	}
#endif
	...
	if(!thrown && no_obj) {      /* attack with bare hands */
		...
		if (/* if you strike with a bow... */
			...
			hittxt = TRUE;
			  ...
			  (Role_if(PM_ENFORCER) && is_lightsaber(obj)) &&
			  ((wtype = uwep_skill_type()) != P_NONE &&
			    P_SKILL(wtype) >= P_SKILLED) &&
			  ((monwep = MON_WEP(mon)) != 0 &&
